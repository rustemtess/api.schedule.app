...
function post(array $arr): void {
    ...
    $url = 'http://schedule.com/api/whatsapp/only';
...
function createDate(string $date, string $time, array $data): void {
    ...
    if (!isDate($date)) {
        ...
        if (!$query->execute()) {
            throw new Exception('Ошибка при выполнении запроса: ' . $query->error);
        }
    }
    ...
    if ($dateId && $timeId) {
        $query = $db->prepare("INSERT INTO `meeting` (`date_id`, `time_id`) VALUES (?, ?)");
        if (!$query) {
            throw new Exception('Ошибка при подготовке запроса: ' . $db->error);
        }
        ...
        $query->bind_param('isssiis', $meetId, $time, $data[0], $meetFile, $data[3], $userId, $data[4]);
        if (!$query->execute()) {
            throw new Exception('Ошибка при выполнении запроса: ' . $query->error);
        }
